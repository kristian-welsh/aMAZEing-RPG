package com.maze.src.maze.edge {
	...
	public class MazeEdgeDrawer {
		...
		private function placeVWallIfAtMazeEdge(currentPoint:Point):void {
			...
			if (currentPoint.x == Settings.NUM_COLUMNS - 1) {
				addVWallAt(new Point(Settings.NUM_COLUMNS, currentPoint.y));
			}
		}
		...
		private function placeHWallIfAtMazeEdge(currentPoint:Point):void {
			...
			if (currentPoint.y == Settings.NUM_ROWS - 1) {
				addHWallAt(new Point(currentPoint.x, Settings.NUM_ROWS));
			}
		}
	}
}